...
with open(r'C:\Users\r14ale\Desktop\Scrapping_ABB\Images.txt', encoding="utf8") as file:
    ...
    with open(f"C:\\Users\\r14ale\\Desktop\\Scrapping_ABB\\{subfolder_name}\\Image.csv", mode='w', encoding='utf-8') as ContentData:
        ...
        for image in Images_Location:
            Image = image['src']
            data.writerow([Index,"Image Link",'<tr><th class="tg-0lax">'+Image+'</th></tr>'])
            Index = Index+1
...
with open(r'C:\Users\r14ale\Desktop\Scrapping_ABB\General_Information.txt', encoding="utf8") as file_0:
    ...
    with open(f"C:\\Users\\r14ale\\Desktop\\Scrapping_ABB\\{subfolder_name}\\General_Information.csv", mode='w', encoding='utf-8') as ContentData_0:
        ...
        data_0.writerow(["Index","Item","Description","Final_Data"])
        for item,desc in zip(Main,Desc):
            ...
            for Descriptions in Description:
                ...
                data_0.writerow([Index_GI,'<tr><th class="tg-0lax">'+str(Item)+'</th>','<tr><th class="tg-0lax">'+str(Descriptions)+'</th></tr>','<tr><th class="tg-0lax">'+str(Item)+'</th><th class="tg-0lax">'+str(Descriptions)+'</th></tr>'])
                Index_GI = Index_GI + 1
...
df = pd.read_csv(f"C:\\Users\\r14ale\\Desktop\\Scrapping_ABB\\{subfolder_name}\\Sample_Data.csv")

# Combine data from Column D into a single column
df['Combined_Column'] = df['Final_Data']

# Display the resulting DataFrame (optional)
print(df[['Combined_Column']])

# Convert the values in the Final_Data column to strings
df['Final_Data'] = df['Final_Data'].astype(str)

# Drop rows with NaN values
df_cleaned = df.dropna()
# Combine data from rows 1 to 5 in Column D
combined_data = ' '.join(df_cleaned.loc[1:5, 'Final_Data'])

# Save the combined data to a CSV file
df_combined = pd.DataFrame({'Short Description': [combined_data]})
#df_combined.to_csv(f"C:\\Users\\r14ale\\Desktop\\Scrapping_ABB\\{subfolder_name}\\Final_Sample_Data_1.csv", index=False)  # You can choose a different file name if 
# print(f"Combined data: {combined_data}")

df_combined.to_csv(f"C:\\Users\\r14ale\\Desktop\\Scrapping_ABB\\{subfolder_name}\\Final_Sample_Data.csv", index=False)  # You can choose a different file name if needed




# Read the CSV file (replace 'your_file.csv' with the actual file path)
df = pd.read_csv(f"C:\\Users\\r14ale\\Desktop\\Scrapping_ABB\\{subfolder_name}\\Final_Sample_Data.csv")

# Add sentences before point A (modify as needed)
sentences = ['<table class="tg"><thead>', '</thead></table>']

# Modify the data in Column A
df['Short Description'] = sentences[0]+df['Short Description']+sentences[1]

Product_Code = ["SH201-C16"]
df['Product_Code'] = Product_Code

# Save the modified DataFrame back to the CSV file
df.to_csv(f"C:\\Users\\r14ale\\Desktop\\Scrapping_ABB\\{subfolder_name}\\Final_Sample_Data.csv", index=False)  # You can choose a different file name if needed
